# Feature Spec: Clean map output: show only map image with short label

## Requirements

When a map is sent to Telegram, it should only show the map image and a very short label describing what the map shows (e.g. 'Tree cover loss - Spain'). Currently extra text/context is sent alongside the map which clutters the output. Remove all extra text so only the map image and a concise caption are sent.

## Discussion / Context

### Comment by @craigmmills on 2026-02-11T02:08:28Z

Spec started locally at: /Users/craigmills/Projects/project-zeno/main/.specs/5-clean-map-output-show-only-map-image-with-short-la
